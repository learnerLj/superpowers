...
这个合同统一定义单会话与多会话复盘、lesson 字段、证据状态、知识晋级和 Raw retention。来源 reference 负责解释数据格式；本文件负责决定复盘结果能写成什么、能落到哪里、能否进入删除候选。ledger 列名保持英文；对用户说「经验教训」，不要说「课」。怎么说，以 `SKILL.md` 为准。
...
候选清单生成后，询问用户或等待用户对精确 session ID 或文件范围的明确指令，才能进入 `approved`。「以后会删」「太大了」「总结完再删」只打开路径，不是现在删除。宽泛的旧授权不能替代这一步。skill 不提供默认批量删除命令。写进月度档案不等于总结完成，也不等于可删。
