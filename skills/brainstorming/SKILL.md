...
<HARD-GATE>
在长任务的目标、authority、交付物、执行 slice、完成证据和修订条件全部写进同一份 spec 之前，不要开始执行。只有本 skill 正确触发后才应用此门槛；短小、自包含的任务不需要 spec。用户只要 spec、brainstorm 或先规划时，只写 spec 文件，不得改正文、vault 正式笔记或实现代码。
</HARD-GATE>
...
每个会影响 slice 或最终完成判断的 requirement、invariant 或验收标准都使用 criterion ID，并在执行前声明：

- **要证明的事实**：可观察、可证伪的完成条件；
- **Oracle**：能够区分该事实成立与不成立的检查、命令、产物或 observation；
- **通过条件**：执行验证前确定的预期结果或阈值；
...
- **目标文件**以及每个文件的职责；
- **系统组成与数据流**；
- 向成熟系统接入新模块或跨模块能力时，在系统数据流中补充简短的**现有系统融入检查**；
...
Adapter、provider、plugin、client 或其它跨模块接入任务必须在系统组成与数据流中简要写明：

- **现有 owner**：相关状态、生命周期和通用能力由谁负责；
- **复用入口**：本次改动通过哪个既有入口或 extension point 接入；
- **新增责任**：新模块只拥有哪些项目特有能力；
- **平行系统检查**：是否新增第二份状态、状态机、重试、缓存或生命周期；如有，为什么现有 owner 无法承担。

独立功能不增加这一段。只有存在多个 owner 或真实框架缺口时才展开责任表；实施中发现未写入 spec 的缺口时，停止实现并修订 spec。
...
- 用户说「不要 checkpoint」「不管 checkpoint」「全部执行完」「不要停」「直接都改了」或同义指令之后：规格内部仍按 slice 记录进度，但对话中禁止再停下来等用户确认每一个 slice。连续执行到交付，或碰到密钥、笔记冲突、未批准的软件实现预授权等硬闸门。禁止把「改动超过 3 个文件必须停下等待」当成全局规则。
